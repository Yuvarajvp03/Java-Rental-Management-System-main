...
public class RegisterContr {
    ...
    public void signUpButtonMouseClicked(int roleNumber) { //sign up
        ...
        if (register.getCheckRegister()) {
        	String generatedPassword = Admin.generatePassword();
            //generate request and store into database
            Request request = new Request(roleNumber, currentFrame.getNameEntered(), currentFrame.getEmailEntered(), currentFrame.getPhoneEntered(), currentFrame.getPassword());
