...
import property.PropertyInterface;

public class Agent extends Account implements PropertyInterface{
    ...
    public boolean createAccount() {
        try{
            ...
            String query = "insert into agent (agent_name, agent_phone, agent_email, agent_username, agent_password) VALUES ('" +
    ...
    public void addProperty(Property property,ArrayList<String> facilityList) { //save new property and facility into database
        try {
            PreparedStatement stmt=DatabaseConnection.getConnection().prepareStatement("insert into property (property_name, property_address, property_type, property_price, property_project, property_size, property_numRoom, property_numBathroom, property_status, property_agent) values (?,?,?,?,?,?,?,?,?,?);");
    ...
    public ArrayList<Property> viewOwnProperty(){ //Get all my properties from database
        ...
        try{
            Statement statement = DatabaseConnection.getConnection().createStatement();
            String sql = "select * from property where property_agent=" + getID() + ";";
            ...
            while(result.next()){
                ...
                property.setOwnerID(result.getInt("property_agent"));
    ...
    public boolean login(String usernameEntered, String passwordEntered) { //check username password in database
        try{
            ...
            String sql = "select * from agent where agent_username='"+usernameEntered
                    +"' and agent_password='"+ passwordEntered+"'";
    ...
    public void editProfile() { //update edited account into database
        try {
            ...
            String sql = "update agent " +
                    "set agent_name = '" + getName() + "'" +
                    ", agent_phone = " + getPhone() +
                    ", agent_email = '" + getEmail() + "'" +
                    ", agent_username = '" + getUsername() + "'" +
                    ", agent_password = '" + getPassword() + "'" +
                    " where agent_id = " + getID() +
                    ";";
            statement.executeUpdate(sql);
        }catch (Exception ex){
            ex.printStackTrace();
    ...
    public static ArrayList<String> getNamePhone(int ID){ //get agent's name and phone to display to users
        try{
            Statement statement = database.DatabaseConnection.getConnection().createStatement();
            String sql = "select agent_name,agent_phone from agent where agent_id = "+ ID;
            ResultSet resultSet = statement.executeQuery(sql);
            ArrayList<String> namePhone = new ArrayList<>();

            if (resultSet.next()){
                namePhone.add(resultSet.getString(1));
                namePhone.add(Long.toString(resultSet.getLong(2)));
                return namePhone;
            }
